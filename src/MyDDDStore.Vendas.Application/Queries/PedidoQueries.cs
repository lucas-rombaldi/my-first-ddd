...
namespace MyDDDStore.Vendas.Application.Queries
{
    public class PedidoQueries : IPedidoQueries
    {
        ...
        {
            var pedido = await _pedidoRepository.FindDraftByClienteID(clienteId);
        ...
        {
            var pedidos = await _pedidoRepository.FindByClientID(clienteId);

            pedidos.Where(p => p.PedidoStatus == PedidoStatus.Pago || p.PedidoStatus == PedidoStatus.Cancelado)
